plppdo.on('domChanged', () => {
    if (document.getElementById('khanecoTab')) return;
    ...
    section.id = 'khanecoTab';
    section.className = '_1ozlbq6';
    section.innerHTML = '<h2 class="_18undph9">Khaneco</h2>';
    ...
    devTab.querySelector('a').addEventListener('click', (e) => {
        e.preventDefault();
        window.khanecoWin = window.open("", "_blank");
        if (window.khanecoWin) {
            window.khanecoWin.document.write(`
                <html>
                <head>
                    <title>Khaneco Developer</title>
                    <style>
                        body { 
...
window.createToggle = function(name, desc, varName, toggled = false) {
    if (!window.khanecoWin || window.khanecoWin.closed) return;

    const toggleContainer = window.khanecoWin.document.getElementById('toggles');
...
window.debug = function(message) {
    if (!window.khanecoWin || window.khanecoWin.closed || !window.debugMode) return;
    
    const debugBox = window.khanecoWin.document.getElementById('debugBox');
    if (debugBox) {
